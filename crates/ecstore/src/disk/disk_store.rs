...
const DISK_HEALTH_OK: i64 = 0;
const DISK_HEALTH_FAULTY: i64 = 1;
...
impl DiskHealthTracker {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            status: AtomicI64::new(DISK_HEALTH_OK),
...
/// DiskStoreWrapper wraps a DiskStore with health tracking capabilities.
/// This is similar to Go's xlStorageDiskIDCheck.
#[derive(Debug, Clone)]
pub struct DiskStoreWrapper {
...
impl DiskStoreWrapper {
    /// Create a new DiskStoreWrapper
    pub fn new(disk: Arc<Disk>, health_check: bool) -> Self {
        Self {
            disk,
    ...
    async fn monitor_disk_writable(disk: Arc<Disk>, health: Arc<DiskHealthTracker>, cancel_token: CancellationToken) {
        const CHECK_EVERY: Duration = Duration::from_secs(15);
        const SKIP_IF_SUCCESS_BEFORE: Duration = Duration::from_secs(5);
        ...
        loop {
            tokio::select! {
                ...
                _ = interval.tick() => {
                    ...
                    if Self::perform_health_check(disk.clone(), &health).await.is_err() {
                        // Health check failed, disk is considered faulty
                        health.set_faulty();
                        health.decrement_waiting(); // Balance the increment from failed operation
    ...
    async fn perform_health_check(disk: Arc<Disk>, _health: &DiskHealthTracker) -> Result<()> {
        const TEST_BUCKET: &str = ".rustfs.sys/tmp";
        let test_filename = format!("health-check-{}", Uuid::new_v4());
        let test_data = Bytes::from(vec![42u8; 2048]);

        // Try to write test data
        if let Err(e) = disk.write_all(TEST_BUCKET, &test_filename, test_data.clone()).await {
            if matches!(e, DiskError::FaultyDisk) {
                return Err(e);
            }
            return Err(e);
        }
        ...
        let read_data = match disk.read_all(TEST_BUCKET, &test_filename).await {
            Ok(data) => data,
            Err(e) => {
                if matches!(e, DiskError::FaultyDisk) {
                    return Err(e);
    ...
    async fn monitor_disk_status(disk: Arc<Disk>, health: Arc<DiskHealthTracker>, cancel_token: CancellationToken) {
        ...
        loop {
            tokio::select! {
                ...
                _ = interval.tick() => {
                    ...
                    match Self::perform_health_check(disk.clone(), &health).await {
                        Ok(()) => {
    ...
    async fn check_disk_stale(&self) -> Result<()> {
        ...
        let current_disk_id = self.disk_id.read().await;
        if let Some(current_id) = *current_disk_id {
            if current_id != stored_disk_id {
    ...
    pub async fn track_disk_health<T, F, Fut>(&self, operation: F) -> Result<T>
    where
        F: FnOnce() -> Fut,
...
impl DiskAPI for DiskStoreWrapper {
    ...
    async fn get_disk_id(&self) -> Result<Option<Uuid>> {
        self.track_disk_health(|| async { self.disk.get_disk_id().await }).await
    }
    ...
    async fn make_volume(&self, volume: &str) -> Result<()> {
        self.track_disk_health(|| async { self.disk.make_volume(volume).await }).await
    }

    async fn make_volumes(&self, volumes: Vec<&str>) -> Result<()> {
        self.track_disk_health(|| async { self.disk.make_volumes(volumes).await })
            .await
    }

    async fn list_volumes(&self) -> Result<Vec<VolumeInfo>> {
        self.track_disk_health(|| async { self.disk.list_volumes().await }).await
    }

    async fn stat_volume(&self, volume: &str) -> Result<VolumeInfo> {
        self.track_disk_health(|| async { self.disk.stat_volume(volume).await }).await
    }

    async fn delete_volume(&self, volume: &str) -> Result<()> {
        self.track_disk_health(|| async { self.disk.delete_volume(volume).await })
            .await
    }

    async fn walk_dir<W: tokio::io::AsyncWrite + Unpin + Send>(&self, opts: WalkDirOptions, wr: &mut W) -> Result<()> {
        self.track_disk_health(|| async { self.disk.walk_dir(opts, wr).await }).await
    }
    ...
    async fn delete_version(
        ...
        self.track_disk_health(|| async { self.disk.delete_version(volume, path, fi, force_del_marker, opts).await })
            .await
    }
    ...
    async fn delete_paths(&self, volume: &str, paths: &[String]) -> Result<()> {
        self.track_disk_health(|| async { self.disk.delete_paths(volume, paths).await })
            .await
    }

    async fn write_metadata(&self, org_volume: &str, volume: &str, path: &str, fi: FileInfo) -> Result<()> {
        self.track_disk_health(|| async { self.disk.write_metadata(org_volume, volume, path, fi).await })
            .await
    }

    async fn update_metadata(&self, volume: &str, path: &str, fi: FileInfo, opts: &UpdateMetadataOpts) -> Result<()> {
        self.track_disk_health(|| async { self.disk.update_metadata(volume, path, fi, opts).await })
            .await
    }
    ...
    async fn read_version(
        ...
        self.track_disk_health(|| async { self.disk.read_version(org_volume, volume, path, version_id, opts).await })
            .await
    }

    async fn read_xl(&self, volume: &str, path: &str, read_data: bool) -> Result<RawFileInfo> {
        self.track_disk_health(|| async { self.disk.read_xl(volume, path, read_data).await })
            .await
    }
    ...
    async fn rename_data(
        ...
        self.track_disk_health(|| async { self.disk.rename_data(src_volume, src_path, fi, dst_volume, dst_path).await })
            .await
    }

    async fn list_dir(&self, origvolume: &str, volume: &str, dir_path: &str, count: i32) -> Result<Vec<String>> {
        self.track_disk_health(|| async { self.disk.list_dir(origvolume, volume, dir_path, count).await })
            .await
    }

    async fn read_file(&self, volume: &str, path: &str) -> Result<crate::disk::FileReader> {
        self.track_disk_health(|| async { self.disk.read_file(volume, path).await })
            .await
    }

    async fn read_file_stream(&self, volume: &str, path: &str, offset: usize, length: usize) -> Result<crate::disk::FileReader> {
        self.track_disk_health(|| async { self.disk.read_file_stream(volume, path, offset, length).await })
            .await
    }

    async fn append_file(&self, volume: &str, path: &str) -> Result<crate::disk::FileWriter> {
        self.track_disk_health(|| async { self.disk.append_file(volume, path).await })
            .await
    }

    async fn create_file(&self, origvolume: &str, volume: &str, path: &str, file_size: i64) -> Result<crate::disk::FileWriter> {
        self.track_disk_health(|| async { self.disk.create_file(origvolume, volume, path, file_size).await })
            .await
    }

    async fn rename_file(&self, src_volume: &str, src_path: &str, dst_volume: &str, dst_path: &str) -> Result<()> {
        self.track_disk_health(|| async { self.disk.rename_file(src_volume, src_path, dst_volume, dst_path).await })
            .await
    }

    async fn rename_part(&self, src_volume: &str, src_path: &str, dst_volume: &str, dst_path: &str, meta: Bytes) -> Result<()> {
        self.track_disk_health(|| async { self.disk.rename_part(src_volume, src_path, dst_volume, dst_path, meta).await })
            .await
    }

    async fn delete(&self, volume: &str, path: &str, opt: DeleteOptions) -> Result<()> {
        self.track_disk_health(|| async { self.disk.delete(volume, path, opt).await })
            .await
    }

    async fn verify_file(&self, volume: &str, path: &str, fi: &FileInfo) -> Result<CheckPartsResp> {
        self.track_disk_health(|| async { self.disk.verify_file(volume, path, fi).await })
            .await
    }

    async fn check_parts(&self, volume: &str, path: &str, fi: &FileInfo) -> Result<CheckPartsResp> {
        self.track_disk_health(|| async { self.disk.check_parts(volume, path, fi).await })
            .await
    }

    async fn read_parts(&self, bucket: &str, paths: &[String]) -> Result<Vec<ObjectPartInfo>> {
        self.track_disk_health(|| async { self.disk.read_parts(bucket, paths).await })
            .await
    }

    async fn read_multiple(&self, req: ReadMultipleReq) -> Result<Vec<ReadMultipleResp>> {
        self.track_disk_health(|| async { self.disk.read_multiple(req).await }).await
    }

    async fn write_all(&self, volume: &str, path: &str, data: Bytes) -> Result<()> {
        self.track_disk_health(|| async { self.disk.write_all(volume, path, data).await })
            .await
    }

    async fn read_all(&self, volume: &str, path: &str) -> Result<Bytes> {
        self.track_disk_health(|| async { self.disk.read_all(volume, path).await })
            .await
    }
    ...
    async fn disk_info(&self, opts: &DiskInfoOptions) -> Result<DiskInfo> {
        if opts.noop && opts.metrics {
            ...
            // Add health metrics
            info.metrics.total_waiting = self.health.waiting_count() as u32;
            if self.health.is_faulty() {
                return Err(DiskError::FaultyDisk);
            }
            return Ok(info);
        ...
        let result = self.track_disk_health(|| async { self.disk.disk_info(opts).await }).await?;

        if opts.metrics {
            // Update metrics with health info
            // This would need to be implemented based on the actual DiskMetrics structure
        }
